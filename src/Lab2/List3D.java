...
public class List3D<T> {
    ...
    public static void main(String[] args) {
        ...
        List3D<Integer> array = new List3D<>(raw, 3, 3, 3);
    ...
    public List3D(List<List<List<T>>> megaArray3D) {
        this.megaArray3D = megaArray3D;
    }

    public List3D(int size, T t) {
        if (checkerSize(size)) {
    ...
    public List3D(List<T> rawArray, int x, int y, int z) {
     ...
     * @return new 3DVector
     */
    public static List3D<?> CreateFill(int size, Integer t) {
        if (size > 0) {
